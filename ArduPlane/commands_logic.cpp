...
void Plane::use_base_turn_shortcut()
{
    ...
    */
    if (labs(course_err_cd) <= 1000) {
        ...
        gcs().send_text(MAV_SEVERITY_INFO, "Base Turn Shortcut applied");

        auto_state.entering_base_leg = false;
        // reset previous waypoint and make sure target altitude and glide slope are consistent
        prev_WP_loc = current_loc;
        prev_WP_loc.alt = target_altitude.amsl_cm;
        setup_glide_slope();
    }
}
